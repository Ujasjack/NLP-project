...
class POSGuidedAttention(nn.Module):
    ...
    def forward(self, sentence, length, pos_embed):
        ...
        sentence_mask = torch.arange(self.seq_length).unsqueeze(0).repeat(self.batch_size, 1).to(device) >= length.unsqueeze(1)
        if self.verbose:
...
class Attention(nn.Module):
    ...
    def forward(self, sentence, length, pos_embedding, cardinal_phrase_embedding, headline_embedding):
        pos_weights = self.pos_attention_weights(sentence, length, pos_embedding)
        cardinal_phrase_weights = self.cardinal_phrase_attention_weights(sentence, length, cardinal_phrase_embedding)
        headline_weights = self.headline_attention_weights(sentence, length, headline_embedding)
        
        fusion_attention_weights = torch.mean(torch.stack([pos_weights, cardinal_phrase_weights, headline_weights]), dim=0)
        if self.verbose:
...
class POSHAN(nn.Module):
    ...
    def forward(self, headline_embedding, cardinal_phrase_embedding, pos_pattern, body_embedding, body_length):
        ...
        pos_embedding = self.pos_embedding(pos_pattern)
        if self.verbose:
...
path = "poshan_fnc_model.pt"
torch.save(model.state_dict(), path)

model1 = POSHAN(pos_vocab_size, pos_embedding_dim, embedding_dim, lstm_hidden_size, no_layers, batch_size, False).to(device)
model1.to(device)

# Load the state_dict of our saved model (this will update the new instance of our model with trained weights)
model1.load_state_dict(torch.load(f='poshan_fnc_model.pt',map_location=torch.device('cpu')))
model1.eval()

test_file = './Dataset/fnc_test_derived_pos_extracted.csv'
test_df = pd.read_csv(test_file)
test_headline = test_df['Headline'].values
test_pos_pattern = test_df['POS-pattern'].values
test_cardinal_phrase = test_df['Cardinal-phrase'].values
...
x_test_headline = [headline for headline in test_headline]
x_test_body = [split_body(body) for body in test_body]
test_dataset = MyDataset(x_test_headline, x_test_body, test_body_length, test_cardinal_phrase, test_pos_pattern, test_stance)

batch_size = 8

test_loader = DataLoader(test_dataset, shuffle=True, batch_size=batch_size, collate_fn=collate_fn, drop_last=True)

test_acc = 0.00
total_test_samples = 0
y_pred, y_test = [], []
for headline, pos_pattern, cardinal_phrase, body, body_length, labels in tqdm(test_loader):
    headline, cardinal_phrase, pos_pattern, body, body_length, labels = headline.to(device), cardinal_phrase.to(device), pos_pattern.to(device), body.to(device), body_length.to(device), labels.to(device)
    output = model(headline, cardinal_phrase, pos_pattern, body, body_length)
    _, predicted = torch.max(output, 1)
    y_pred += predicted
...
print("Accuracy", test_acc/total_test_samples)
